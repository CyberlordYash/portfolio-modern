        ...
        {data.map((item, index) => (
          <div key={index} className="flex justify-start gap-0 md:gap-10 pt-8 md:pt-24">
            ...
            <div className="sticky top-36 z-40 flex w-14 md:w-[220px] shrink-0 self-start items-center md:flex-row">
              {/* square dot */}
              <div className="absolute left-3 flex h-10 w-10 items-center justify-center bg-[#ffffff] dark:bg-black/70">
              ...
              <div className="hidden md:flex pl-20">
                <span className="border border-black/15 dark:border-white/15 bg-[#ffffff] dark:bg-black/70 px-2.5 py-1 font-mono text-[8px] uppercase tracking-[0.35em] text-black/60 dark:text-white/60 whitespace-nowrap">
                  {item.title}
                </span>
              </div>
            </div>
